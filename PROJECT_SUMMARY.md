...
- [ ] **P2P transfer — untested scenarios.** Verified so far: same-WiFi P2P (full ICE→data-channel→ack), metadata-only (giftcard link) shares correctly skipping P2P, and the offline/failed-P2P encrypted rescue fallback (confirmed via DynamoDB: correct ciphertext length, exact 72h TTL, decrypts correctly on read, row cleared on consumption). Still to verify:
