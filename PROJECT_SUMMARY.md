...
**Last updated:** 2026-08-25 (Live redemption notifications - non-owners can now redeem a shared coupon at all via a member-scoped endpoint, redemptions are atomic, and full/partial redeems notify everyone who can see the coupon. Fixed a latent notification-ordering bug that was silently dropping coupon codes from shares, gave notifications real retention (30-day TTL + a created_at GSI - two manual DynamoDB console steps, see setup section). Also fixed: editing a shared coupon's balance/etc. wasn't live-refreshing the owner's own other screens (pushCouponUpdated deliberately skips the actor - nothing locally called bump() after an edit save); and a coupon's *code* silently going stale for recipients after the owner edited it, since the code never touches the server and nothing redelivered it - new coupon_code_sync fallback path, silent by design.)
