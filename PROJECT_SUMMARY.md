...
**Last updated:** 2026-08-24 (Live "coupon used" notifications — non-owners can now redeem a shared coupon at all, via a new member-scoped redeem endpoint; notifies every group the coupon is shared to regardless of who redeemed it, reusing the NotificationsContext revision-bump plumbing)
...
server/src/
  ...
  routes/
    ...
    coupons.ts            — CRUD for coupon metadata + POST /:id/redeem (owner) (auth-protected)
...
- [x] **Live "coupon used" notifications** — marking a shared coupon `used` (full redeem, or a partial redeem that drains the balance) notifies every other member of every group that coupon is shared to, **whether the owner or a non-owner did the redeeming**. New notification type `coupon_used`; flows through the existing generic notification-panel/banner/OS-notification rendering unchanged.
...
- [ ] **Rescue-code lookup scans only recent notifications** — `rescueCode` (`server/src/repositories/notifications.ts`) locates the recipient's `group_share` row via `getNotificationsForUser`, which returns only the 50 newest. A recipient with heavy notification traffic could have that row pushed out of the window between the share and the rescue, so the rescue no-ops and they never receive the code. Fix by querying the specific `group_id`/`coupon_id` rather than scanning the recent page. (TODO comment marks the spot.)
